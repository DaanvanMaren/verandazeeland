...
export default async function ZakelijkHorecaPage() {
  const c = await getContent('zakelijk-horeca')
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', background: '#f6f0e6' }}>
      ...
      <div style={{ padding: '20px 40px 0', fontSize: 13, color: '#8a94a0' }}>
        <a className="nav-a" href="/">Home</a> › <a className="nav-a" href="/zakelijk">Zakelijk</a> ›{' '}
        <span style={{ color: '#082a52', fontWeight: 600 }}>Horeca &amp; Leisure</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 44, padding: '24px 40px 56px', alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', borderRadius: 22, overflow: 'hidden', minHeight: 420, background: '#082a52' }}>
          <img src={img(c.heroImage, c.heroFallback ?? '/uploads/foto-horeca.jpg')} alt={c.heroTitle} style={cover} />
        </div>
      </div>
