...
export default async function ZakelijkProjectbouwPage() {
  const c = await getContent('zakelijk-projectbouw')
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', background: '#f6f0e6' }}>
      ...
      <div style={{ padding: '20px 40px 0', fontSize: 13, color: '#8a94a0' }}>
        ...
        <span style={{ color: '#082a52', fontWeight: 600 }}>Projectbouw & Ontwikkeling</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 44, padding: '24px 40px 56px', alignItems: 'center' }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'Archivo', fontWeight: 900, fontSize: 52, lineHeight: 1, margin: '0 0 20px', letterSpacing: '-1.6px' }}>{c.heroTitle}</h1>
      ...
      {/* quote band */}
      {c.quote && (
        <div style={{ background: '#082a52', color: '#fff', margin: '56px 40px', borderRadius: 26, padding: 56, textAlign: 'center' }}>
          <p style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 26, lineHeight: 1.3, margin: '0 auto', maxWidth: 760, letterSpacing: '-.5px' }}>{c.quote}</p>
        </div>
      )}
