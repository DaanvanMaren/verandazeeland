...
export default async function TuinkamerHellendDakPage() {
  const c = await getContent('tuinkamer-hellend-dak')
  ...
  return (
    <div className="max-w-[1280px] mx-auto bg-cream">
      ...
      <div className="pt-[20px] px-[40px] max-[600px]:px-[16px] text-[13px] text-mute">
        <a className="nav-a" href="/">Home</a> › <a className="nav-a" href="/tuinkamers">Tuinkamers</a> ›{' '}
        <span className="text-navy font-semibold">Hellend dak</span>
      </div>
      ...
      <div className="grid grid-cols-2 max-[820px]:grid-cols-1 gap-[44px] pt-[24px] px-[40px] pb-[56px] max-[600px]:px-[16px] items-center">
        <div>
          ...
          <h1 className="font-display font-black text-[52px] leading-[1] mb-[20px] tracking-[-1.7px]">{c.heroTitle}</h1>
      ...
      <div className="grid grid-cols-[1.15fr_1fr] max-[820px]:grid-cols-1 gap-[48px] pt-[20px] px-[40px] pb-[56px] max-[600px]:px-[16px] items-center">
        ...
        <div className="relative rounded-[18px] overflow-hidden min-h-[360px] bg-navy">
      ...
      <div className="pt-[40px] px-[40px] pb-[56px] max-[600px]:px-[16px] bg-line">
        <h2 className="font-display font-extrabold text-[32px] mb-[30px] tracking-[-1px]">{c.featuresHeading}</h2>
      ...
      {/* praktijk / gallery */}
      <div className="py-[56px] px-[40px] max-[600px]:px-[16px]">
        <h2 className="font-display font-extrabold text-[28px] mb-[24px] tracking-[-0.8px]">{c.galleryHeading}</h2>
      ...
      <div className="bg-navy text-white mb-[56px] mx-[40px] max-[600px]:mx-[16px] rounded-[26px] p-[56px] max-[600px]:p-[26px] grid grid-cols-2 max-[820px]:grid-cols-1 gap-[48px] items-center">
        <div>
          <span className="text-gold font-extrabold text-[13px] tracking-[1.5px] uppercase">{c.altLabel}</span>
          <h2 className="font-display font-extrabold text-[32px] mt-[12px] mb-[14px] tracking-[-1px]">{c.altTitle}</h2>
          <p className="text-[16.5px] leading-[1.7] text-sky mb-[24px]">{c.altText}</p>
          <a className="btn-y bg-gold text-navy font-extrabold text-[15px] py-[14px] px-[26px] rounded-full inline-block" href="/tuinkamer-vlak-dak">{c.altCta}</a>
        </div>
        <div className="relative rounded-[16px] overflow-hidden min-h-[300px] bg-navy-light">
          <img src={img(c.altImage, c.altImageFallback ?? '')} alt={c.altTitle ?? ''} className={cover} />
        </div>
      </div>
