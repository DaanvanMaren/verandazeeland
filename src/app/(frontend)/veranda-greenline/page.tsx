...
export default async function GreenlinePage() {
  const c = await getContent('veranda-greenline')
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', background: '#f6f0e6' }}>
      ...
      <div style={{ padding: '20px 40px 0', fontSize: 13, color: '#8a94a0' }}>
        ...
        <span style={{ color: '#082a52', fontWeight: 600 }}>Greenline</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 44, padding: '24px 40px 48px', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <a className="btn-n" href="/offerte" style={{ background: '#082a52', color: '#fff', fontWeight: 700, fontSize: 15, padding: '15px 26px', borderRadius: 999 }}>{c.heroButtonPrimary}</a>
      ...
      {/* specs strip */}
      <div style={{ margin: '0 40px', background: '#fff', border: '1px solid #eee4d2', borderRadius: 18, overflow: 'hidden', display: 'grid', gridTemplateColumns: `repeat(${c.specs?.length || 5},1fr)` }}>
        {c.specs?.map((row, i) => (
          <div key={row.id} style={{ padding: '22px 24px', borderRight: i < (c.specs?.length ?? 0) - 1 ? '1px solid #eee4d2' : 'none' }}>
      ...
      <div style={{ background: '#082a52', color: '#fff', margin: '56px 40px', borderRadius: 26, padding: 56, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
        <div>
          <span style={{ color: '#ffcf33', fontWeight: 800, fontSize: 13, letterSpacing: '1.5px', textTransform: 'uppercase' }}>{c.configLabel}</span>
          <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 32, margin: '12px 0 14px', letterSpacing: '-1px' }}>{c.configTitle}</h2>
          <p style={{ fontSize: '16.5px', lineHeight: 1.7, color: '#c9d6e6', margin: '0 0 24px' }}>{c.configText}</p>
          <a className="btn-y" href="/accessoires" style={{ background: '#ffcf33', color: '#082a52', fontWeight: 800, fontSize: 15, padding: '14px 26px', borderRadius: 999, display: 'inline-block' }}>{c.configCta}</a>
        </div>
        <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', minHeight: 300, background: '#33547d' }}>
          <img src={img(c.configImage, c.configImageFallback ?? '')} alt={c.configTitle ?? ''} style={cover} />
        </div>
      </div>
      ...
      <div style={{ padding: '20px 40px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gridAutoRows: 230, gap: 16 }}>
          {c.gallery?.map((row, i) => (
            <div key={row.id} style={{ gridRow: i === 0 ? 'span 2' : undefined, position: 'relative', borderRadius: 18, overflow: 'hidden', background: '#082a52' }}>
      ...
      <div style={{ padding: '20px 40px 56px' }}>
        <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 28, margin: '0 0 24px', letterSpacing: '-.8px' }}>{c.otherTitle}</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 18 }}>
          {c.otherModels?.map((row) => (
