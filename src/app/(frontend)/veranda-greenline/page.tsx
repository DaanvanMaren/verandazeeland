  ...
  return (
    <div className="max-w-[1280px] mx-auto bg-cream">
      ...
      <div className="pt-[20px] px-[40px] pb-0 max-[600px]:px-[16px] text-[13px] text-mute">
        ...
        <span className="text-navy font-semibold">Greenline</span>
      </div>
      ...
      {/* specs strip */}
      <div className="mx-[40px] max-[600px]:mx-[16px] bg-white border border-line-alt rounded-[18px] overflow-hidden grid" style={{ gridTemplateColumns: `repeat(${c.specs?.length || 5},1fr)` }}>
        {c.specs?.map((row, i) => (
        ...
        <div className="grid grid-cols-4 max-[820px]:grid-cols-1 gap-[18px]">
          {c.features?.map((row) => (
            <div key={row.id} className="lift bg-panel rounded-[18px] p-[26px] shadow-[0_8px_26px_-20px_rgba(8,42,82,0.4)]">
              ...
              <p className="text-[14.5px] text-taupe leading-[1.6] m-0">{row.text}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-navy text-white my-[56px] mx-[40px] max-[600px]:mx-[16px] rounded-[26px] p-[56px] max-[600px]:p-[26px] grid grid-cols-2 max-[820px]:grid-cols-1 gap-[48px] items-center">
        <div>
          <span className="text-gold font-extrabold text-[13px] tracking-[1.5px] uppercase">{c.configLabel}</span>
          <h2 className="font-display font-extrabold text-[32px] mt-[12px] mb-[14px] tracking-[-1px]">{c.configTitle}</h2>
          <p className="text-[16.5px] leading-[1.7] text-sky mb-[24px]">{c.configText}</p>
          <a className="btn-y bg-gold text-navy font-extrabold text-[15px] py-[14px] px-[26px] rounded-full inline-block" href="/accessoires">{c.configCta}</a>
        </div>
        <div className="relative rounded-[16px] overflow-hidden min-h-[300px] bg-navy-light">
          <img src={img(c.configImage, c.configImageFallback ?? '')} alt={c.configTitle ?? ''} className={cover} />
        </div>
      </div>
      ...
      <div className="pt-[20px] px-[40px] pb-[40px] max-[600px]:px-[16px]">
        ...
        <div className="grid grid-cols-[2fr_1fr] max-[820px]:grid-cols-1 auto-rows-[230px] gap-[16px]">
          {c.gallery?.map((row, i) => (
      ...
      <div className="pt-[20px] px-[40px] pb-[56px] max-[600px]:px-[16px]">
        <h2 className="font-display font-extrabold text-[28px] mb-[24px] tracking-[-0.8px]">{c.otherTitle}</h2>
        <div className="grid grid-cols-4 max-[820px]:grid-cols-1 gap-[18px]">
