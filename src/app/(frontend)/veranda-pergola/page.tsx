...
export default async function VerandaPergolaPage() {
  const c = await getContent('veranda-pergola')
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', background: '#f6f0e6' }}>
      ...
      <div style={{ padding: '20px 40px 0', fontSize: 13, color: '#8a94a0' }}>
        <a className="nav-a" href="/">Home</a> › <a className="nav-a" href="/veranda-s">Veranda's</a> › <span style={{ color: '#082a52', fontWeight: 600 }}>Pergola</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 44, padding: '24px 40px 48px', alignItems: 'center' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
            {c.heroBadge && (
              <span style={{ background: '#ffcf33', color: '#082a52', fontWeight: 800, fontSize: 12, letterSpacing: 1, textTransform: 'uppercase', padding: '6px 13px', borderRadius: 999 }}>{c.heroBadge}</span>
            )}
            {c.heroPrice && (
              <span style={{ background: '#eef4fb', color: '#0053a1', fontWeight: 700, fontSize: 13, padding: '6px 12px', borderRadius: 999 }}>{c.heroPrice}</span>
            )}
          </div>
      ...
      <div style={{ padding: '40px 40px 56px', background: '#efe6d4' }}>
        <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 32, margin: '0 0 30px', letterSpacing: '-1px' }}>{c.featuresHeading}</h2>
      ...
      <div style={{ background: '#082a52', color: '#fff', margin: '56px 40px', borderRadius: 26, padding: 56, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
        <div>
          ...
          <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 32, margin: '12px 0 14px', letterSpacing: '-1px' }}>{c.bandTitle}</h2>
          <p style={{ fontSize: '16.5px', lineHeight: 1.7, color: '#c9d6e6', margin: '0 0 24px' }}>{c.bandText}</p>
          <a className="btn-y" href="/accessoires" style={{ background: '#ffcf33', color: '#082a52', fontWeight: 800, fontSize: 15, padding: '14px 26px', borderRadius: 999, display: 'inline-block' }}>{c.bandCta}</a>
        </div>
      ...
      <div style={{ padding: '20px 40px 56px' }}>
        <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 28, margin: '0 0 24px', letterSpacing: '-.8px' }}>{c.relatedHeading}</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 18 }}>
          {c.related?.map((row) => (
