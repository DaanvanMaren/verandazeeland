  ...
  return (
    <div className="max-w-[1280px] mx-auto bg-cream">
      ...
      <div className="pt-[20px] px-[40px] pb-0 max-[600px]:px-[16px] text-[13px] text-mute">
        <a className="nav-a" href="/">Home</a> › <a className="nav-a" href="/veranda-s">Veranda's</a> › <span className="text-navy font-semibold">Pergola</span>
      </div>
      ...
      <div className="grid grid-cols-2 max-[820px]:grid-cols-1 gap-[44px] pt-[24px] px-[40px] pb-[48px] max-[600px]:px-[16px] items-center">
        <div>
          <div className="flex items-center gap-[12px] mb-[18px]">
            {c.heroBadge && (
              <span className="bg-gold text-navy font-extrabold text-[12px] tracking-[1px] uppercase py-[6px] px-[13px] rounded-full">{c.heroBadge}</span>
            )}
            {c.heroPrice && (
              <span className="bg-sky-light text-brand-blue font-bold text-[13px] py-[6px] px-[12px] rounded-full">{c.heroPrice}</span>
            )}
          </div>
      ...
      <div className="bg-navy text-white my-[56px] mx-[40px] max-[600px]:mx-[16px] rounded-[26px] p-[56px] max-[600px]:p-[26px] grid grid-cols-2 max-[820px]:grid-cols-1 gap-[48px] items-center">
        <div>
          ...
          <h2 className="font-display font-extrabold text-[32px] mt-[12px] mb-[14px] tracking-[-1px]">{c.bandTitle}</h2>
          <p className="text-[16.5px] leading-[1.7] text-sky mb-[24px]">{c.bandText}</p>
          <a className="btn-y bg-gold text-navy font-extrabold text-[15px] py-[14px] px-[26px] rounded-full inline-block" href="/accessoires">{c.bandCta}</a>
        </div>
      ...
      <div className="pt-[20px] px-[40px] pb-[56px] max-[600px]:px-[16px]">
        <h2 className="font-display font-extrabold text-[28px] mb-[24px] tracking-[-0.8px]">{c.relatedHeading}</h2>
        <div className="grid grid-cols-4 max-[820px]:grid-cols-1 gap-[18px]">
          {c.related?.map((row) => (
