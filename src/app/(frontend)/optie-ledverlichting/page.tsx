...
export default async function OptieLedverlichtingPage() {
  const c = await getContent('optie-ledverlichting')
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', background: '#f6f0e6' }}>
      ...
      <div style={{ padding: '20px 40px 0', fontSize: 13, color: '#8a94a0' }}>
        <a className="nav-a" href="/">Home</a> › <a className="nav-a" href="/accessoires">Opties</a> ›{' '}
        <span style={{ color: '#082a52', fontWeight: 600 }}>Ledverlichting</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 48, padding: '20px 40px 56px', alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', borderRadius: 18, overflow: 'hidden', minHeight: 340, background: '#082a52' }}>
          <img src={img(c.introImage, c.introImageFallback ?? '')} alt={`${c.heroTitle} detail`} style={cover} />
        </div>
      </div>
