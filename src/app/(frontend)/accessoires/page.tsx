...
export default async function AccessoiresPage() {
  const c = await getContent('accessoires')
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', background: '#f6f0e6' }}>
      ...
      <div style={{ padding: '20px 40px 0', fontSize: 13, color: '#8a94a0' }}>
        <a className="nav-a" href="/">Home</a> › <span style={{ color: '#082a52', fontWeight: 600 }}>Opties</span>
      </div>
      ...
      <div style={{ background: '#082a52', color: '#fff', margin: '0 40px 56px', borderRadius: 26, padding: 56, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
        <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', minHeight: 300, background: '#33547d' }}>
          <img src={img(c.adviesImage, c.adviesImageFallback ?? '')} alt={c.adviesTitle ?? ''} style={cover} />
        </div>
        <div>
          <span style={{ color: '#ffcf33', fontWeight: 800, fontSize: 13, letterSpacing: '1.5px', textTransform: 'uppercase' }}>{c.adviesLabel}</span>
          <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 34, margin: '12px 0 14px', letterSpacing: '-1px' }}>{c.adviesTitle}</h2>
          <p style={{ fontSize: '16.5px', lineHeight: 1.7, color: '#c9d6e6', margin: '0 0 24px' }}>{c.adviesText}</p>
          <a className="btn-y" href="/contact" style={{ background: '#ffcf33', color: '#082a52', fontWeight: 800, fontSize: 15, padding: '14px 26px', borderRadius: 999, display: 'inline-block' }}>{c.adviesCta}</a>
        </div>
      </div>
