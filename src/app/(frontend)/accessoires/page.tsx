  ...
  return (
    <div className="max-w-[1280px] mx-auto bg-cream">
      ...
      <div className="pt-[20px] px-[40px] max-[600px]:px-[16px] text-[13px] text-mute">
        <a className="nav-a" href="/">Home</a> › <span className="text-navy font-semibold">Opties</span>
      </div>
      ...
      <div className="py-[56px] px-[40px] max-[600px]:px-[16px] bg-line">
        <h2 className="font-display font-extrabold text-[34px] mb-[6px] tracking-[-1px]">{c.optionsHeading}</h2>
        {c.optionsIntro && <p className="text-taupe text-[16.5px] mb-[32px]">{c.optionsIntro}</p>}
      ...
      <div className="bg-navy text-white mb-[56px] mx-[40px] max-[600px]:mx-[16px] rounded-[26px] p-[56px] max-[600px]:p-[26px] grid grid-cols-2 max-[820px]:grid-cols-1 gap-[48px] items-center">
        <div className="relative rounded-[16px] overflow-hidden min-h-[300px] bg-navy-light">
          <img src={img(c.adviesImage, c.adviesImageFallback ?? '')} alt={c.adviesTitle ?? ''} className={cover} />
        </div>
        <div>
          <span className="text-gold font-extrabold text-[13px] tracking-[1.5px] uppercase">{c.adviesLabel}</span>
          <h2 className="font-display font-extrabold text-[34px] mt-[12px] mb-[14px] tracking-[-1px]">{c.adviesTitle}</h2>
          <p className="text-[16.5px] leading-[1.7] text-sky mb-[24px]">{c.adviesText}</p>
          <a className="btn-y bg-gold text-navy font-extrabold text-[15px] py-[14px] px-[26px] rounded-full inline-block" href="/contact">{c.adviesCta}</a>
        </div>
      </div>
