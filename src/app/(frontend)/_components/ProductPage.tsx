...
// share the exact same field shape, so one component renders any of them —
// pass the fetched content as `c`. ombouwHref lets a page cross-link its sibling
// model from the "op maat" band (default /accessoires like the Verasol pages).
...
export interface ProductContent {
  ...
  specs?: ({ value?: string | null; label?: string | null } & Row)[] | null
  introTitle?: string | null
  introText?: string | null
  introBullets?: ({ text?: string | null } & Row)[] | null
  introImage?: (number | null) | Media
  introImageFallback?: string | null
  featuresTitle?: string | null
  features?: ({ title?: string | null; desc?: string | null } & Row)[] | null
    ...
    <div className="max-w-[1280px] mx-auto bg-cream">
      ...
      <div className="bg-navy text-white my-[56px] mx-[40px] max-[600px]:mx-[16px] rounded-[26px] p-[56px] max-[600px]:p-[26px] grid grid-cols-2 max-[820px]:grid-cols-1 gap-[48px] items-center">
        <div>
          ...
          <a className="btn-y bg-gold text-navy font-extrabold text-[15px] py-[14px] px-[26px] rounded-full inline-block" href={ombouwHref}>{c.ombouwCta}</a>
        </div>
