...
export default async function SchuifwandSteellookPage() {
  const c = await getContent('schuifwand-steellook')
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', background: '#f6f0e6' }}>
      ...
      <div style={{ padding: '20px 40px 0', fontSize: 13, color: '#8a94a0' }}>
        ...
        <span style={{ color: '#082a52', fontWeight: 600 }}>Steellook</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 44, padding: '24px 40px 56px', alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', borderRadius: 22, overflow: 'hidden', minHeight: 420, background: '#082a52' }}>
          <img src={img(c.heroImage, c.heroFallback ?? '')} alt={c.heroTitle} style={cover} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 48, padding: '20px 40px 56px', alignItems: 'center' }}>
        <div>
          <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 30, margin: '0 0 16px', letterSpacing: '-1px' }}>{c.introHeading}</h2>
        ...
        <div style={{ position: 'relative', borderRadius: 18, overflow: 'hidden', minHeight: 340, background: '#082a52' }}>
          <img src={img(c.introImage, c.introFallback ?? '')} alt={`${c.introHeading ?? ''} detail`} style={cover} />
        </div>
      </div>

      {/* kenmerken & voordelen */}
      <div style={{ padding: '40px 40px 56px', background: '#fff' }}>
        <h2 style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 30, margin: '0 0 30px', letterSpacing: '-1px' }}>{c.featuresHeading}</h2>
