...
export default async function GlazenSchuifwandenPage() {
  const c = await getContent('glazen-schuifwanden')
  ...
  return (
    <div className="max-w-[1280px] mx-auto bg-cream">
      ...
      <div className="pt-[20px] px-[40px] max-[600px]:px-[16px] text-[13px] text-mute">
        <a className="nav-a" href="/">Home</a> › <span className="text-navy font-semibold">Glazen schuifwanden</span>
      </div>
      ...
      <div className="py-[56px] px-[40px] max-[600px]:px-[16px] bg-line">
        <h2 className="font-display font-extrabold text-[34px] mb-[6px] tracking-[-1px]">{c.typesHeading}</h2>
        {c.typesIntro && <p className="text-taupe text-[16.5px] mb-[32px]">{c.typesIntro}</p>}
      ...
      <div className="py-[60px] px-[40px] max-[600px]:px-[16px]">
        <h2 className="font-display font-extrabold text-[34px] text-center mb-[32px] tracking-[-1px]">{c.projectsHeading}</h2>
        <div className="grid grid-cols-[2fr_1fr_1fr] max-[820px]:grid-cols-1 auto-rows-[200px] gap-[16px]">
          {c.projects?.map((row, i) => (
            <div
              key={row.id}
              className={`relative rounded-[18px] overflow-hidden bg-navy${i === 0 ? ' row-span-2' : ''}`}
            >
      ...
      <div className="bg-navy text-white mb-[56px] mx-[40px] max-[600px]:mx-[16px] rounded-[26px] p-[56px] max-[600px]:p-[26px] grid grid-cols-2 max-[820px]:grid-cols-1 gap-[48px] items-center">
        <div className="relative rounded-[16px] overflow-hidden min-h-[300px] bg-navy-light">
          <img src={img(c.comboImage, c.comboImageFallback ?? '')} alt={c.comboTitle ?? ''} className={cover} />
        </div>
        <div>
          <span className="text-gold font-extrabold text-[13px] tracking-[1.5px] uppercase">{c.comboLabel}</span>
          <h2 className="font-display font-extrabold text-[34px] mt-[12px] mb-[14px] tracking-[-1px]">{c.comboTitle}</h2>
          <p className="text-[16.5px] leading-[1.7] text-sky mb-[24px]">{c.comboText}</p>
          <a className="btn-y bg-gold text-navy font-extrabold text-[15px] py-[14px] px-[26px] rounded-full inline-block" href="/veranda-s">{c.comboCta}</a>
        </div>
      </div>
